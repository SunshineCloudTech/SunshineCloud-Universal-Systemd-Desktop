...
```bash
# 拉取镜像（多架构，自动选择对应架构）
docker pull sunshinecloud007/sunshinecloud-universal-systemd-desktop:ubuntu-noble
...
docker run -d --name ubuntu-noble \
  ...
  --cgroupns=host \
  sunshinecloud007/sunshinecloud-universal-systemd-desktop:ubuntu-noble
```
