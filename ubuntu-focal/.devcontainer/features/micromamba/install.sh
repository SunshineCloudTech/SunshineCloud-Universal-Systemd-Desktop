...
# Set environment variables for no-cache operations
export CONDA_PKGS_DIRS="/tmp/conda-pkgs"
export MAMBA_NO_BANNER=1
export CONDA_ALWAYS_SOFTLINK=false
export CONDA_COPY=true
...
micromamba_install_as_user() {
    if [ -n "$*" ]; then
        echo "Installing packages with no cache..."
...
micromamba_create_as_user() {
    ...
    echo "Creating environment with no cache..."
...
if [ -f "${ENV_FILE}" ]; then
    echo "Create env by ${ENV_FILE} with no cache..."
...
rm -rf /tmp/conda-pkgs /tmp/mamba* 2>/dev/null || true
