...
```bash
# 拉取镜像（多架构，自动选择对应架构）
docker pull sunshinecloud007/sunshinecloud-universal-systemd-desktop:debian-bookworm
...
docker run -d --name debian-bookworm \
  ...
  --cgroupns=host \
  sunshinecloud007/sunshinecloud-universal-systemd-desktop:debian-bookworm
```
